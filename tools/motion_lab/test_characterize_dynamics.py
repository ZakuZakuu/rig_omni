...
def _conditioning_capture() -> str:
    ...
    for timestamp, elapsed, command, feedback in (
        ...
        rows.append(f"MLAB,{timestamp},5,2,{elapsed},14000," + ",".join(arrays))
    return header + "\n".join(rows) + "\n"
...
def main() -> int:
    ...
    assert positive_conditioning["description"] == "center -> +8 -> -8 -> center"
    assert negative_conditioning["description"] == "center -> -8 -> +8 -> center"
    assert positive_conditioning["duration_ms"] == 14000
    assert positive_conditioning["offline_profile"]["feasible"] is True
    assert _command_for(positive_conditioning) == "mlab run 5 2 2 8 4000 0 8 30 250"
    assert _command_for(negative_conditioning) == "mlab run 5 2 2 -8 4000 0 8 30 250"
    conditioning_rows = [_row(index, command, feedback) for index, (command, feedback) in enumerate(
        ((0, 100), (27, 130), (-27, 70), (0, 100))
    )]
    ...
    assert conditioning_summary["conditioning_achieved_positive_counts"] == 30.0
    assert conditioning_summary["conditioning_achieved_negative_counts"] == 30.0
    assert conditioning_summary["conditioning_commanded_positive_counts"] == 27.0
    assert conditioning_summary["conditioning_commanded_negative_counts"] == 27.0
    ...
    assert conditioning_summary["conditioning_requested_positive_counts"] > 27.0
    assert conditioning_summary["conditioning_health_required_positive_counts"] == 13.5
    assert conditioning_summary["conditioning_health_required_negative_counts"] == 13.5
    ...
    with tempfile.TemporaryDirectory(prefix="rig-dynamics-conditioning-only-") as directory:
        ...
        assert "mlab run 5 2 2 8 4000 0 8 30 250" in calls
        assert not any(command.startswith("mlab run 4") for command in calls)
        persisted = json.loads((output_dir / "manifest.json").read_text(encoding="utf-8"))
